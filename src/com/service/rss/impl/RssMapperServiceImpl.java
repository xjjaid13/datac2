		...
		try{
			...
			if(rss != null){
				...
				if(rssVO.getRssDetailVOList() != null && rssVO.getRssDetailVOList().size() > 0){
					...
					int i = 2;
					if(rssVO.getRssDetailVOList().size() < i){
						i = rssVO.getRssDetailVOList().size();
					}
					for(;i >= 0;i--){
						RssDetailVO rssDetailVO = rssVO.getRssDetailVOList().get(i);
